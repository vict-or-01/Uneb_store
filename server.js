...
const io = socketIo(server); // Inicializar o Socket.IO
...
const db = mysql.createConnection({
  host: "localhost",
  user: "root",
  password: "29122001", // Substitua pela sua senha
  database: "une_store", // Substitua pelo nome correto do seu banco
});
...
io.on("connection", (socket) => {
  ...
  socket.on("register", (userId) => {
      socket.join(userId); // Associa o usuário ao seu ID
...
app.get("/usuario/meus-anuncios", authenticateToken, (req, res) => {
  ...
  // Consulta para buscar os anúncios do usuário
  ...
  db.query(query, [user_id], (err, results) => {
    ...
    res.json(results); // Retorna os anúncios
  });
});
...
app.post("/likes", (req, res) => {
  ...
  db.query(queryCheck, [userId, productId], (err, results) => {
    ...
    if (results.length > 0) {
      // Atualiza se já existir uma reação
...
app.get("/produtos/filtrar", authenticateToken, (req, res) => {
  ...
  // Consulta básica com filtro de departamento
  let query = `
  ...
  // Adiciona filtro de departamento
  ...
  db.query(query, params, (err, results) => {
    ...
    // Corrige os dados do produto para incluir apenas a primeira foto
    const produtosCorrigidos = results.map((produto) => ({
      ...produto,
      primeiraFoto: produto.imagens ? produto.imagens.split(",")[0] : "imagens/default.jpg",
    }));
...
app.get("/produtos/filtrarPorCategoria", authenticateToken, (req, res) => {
  ...
  // Consulta SQL para filtrar produtos por categoria
  let query = `
  ...
  // Adiciona filtro de categoria, exceto para "todos"
  if (categoria !== "todos") {
  ...
  db.query(query, params, (err, results) => {
    ...
    // Formata os produtos, adicionando a primeira imagem ou um valor padrão
    const produtosCorrigidos = results.map((produto) => ({
      ...produto,
      primeiraFoto: produto.imagens ? produto.imagens.split(",")[0] : "imagens/default.jpg",
    }));
...
app.get("/produtos/buscar", authenticateToken, (req, res) => {
  const userId = req.user.id; // ID do usuário logado
  const termo = req.query.nome; // Termo de busca passado na query string
...
//Back-end: Rota para listar usuários e contagem de produtos
